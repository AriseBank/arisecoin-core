...
if(appConfig.network){
	...
	appConfig.network = networks.aco;
}
...
function startInteractiveMode(scope){
	...
	vorpal
	  ...
	  .action(function(args, callback) {
	    ...
	    var passphrase = require("bip39").generateMnemonic();
			...
			self.log("WIF     - private:",require("arisejs").crypto.getKeys(passphrase).toWIF());
			self.log("Address - public :",require("arisejs").crypto.getAddress(require("arisejs").crypto.getKeys(passphrase).publicKey));
			callback();
	  });
	var account=null;
	vorpal
	...
	vorpal.history('arise-core');

	vorpal
	  .delimiter('arise-core>')
	  .show();
}
