...
node.aco = require('./arise-js');
node.aco.crypto.setNetworkVersion(network.pubKeyHash);
...
node.normalizer = 100000000; // Use this to convert ACO amount to normal value
...
node.randomAccount = function () {
	...
	account.publicKey = node.aco.crypto.getKeys(account.password, network).publicKey;
	account.address = node.aco.crypto.getAddress(account.publicKey, network.pubKeyHash);
